...
class KotlinVersionFormatter(
    ...
    ): String = format.expandContext(PREDEFINED_VARIABLES_LIST.map { (key, value) -> key to value(version) }.toMap())
    ...
    override fun formatToCustomerVersion(
        ...
        customerFormat.expandContext(
            PREDEFINED_COMPONENT_VARIABLES_LIST
                .map { (key, value) -> key to value(format(versionFormat, version), versionPrefix) }
                .toMap()
        )

    override fun matchesFormat(
        format: String,
        version: String
    ): Boolean {
